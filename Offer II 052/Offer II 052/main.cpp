...
public:
    TreeNode* increasingBST(TreeNode* root) {
        TreeNode newHead = TreeNode(-1);
        newHead.right = root;
        TreeNode *p = inorder(root, &newHead, false);
        p->left = nullptr;
        return newHead.right;
    }

    TreeNode* inorder(TreeNode* root, TreeNode* parent, bool left)
    {
        TreeNode* p = parent;
        if (root->left)
        {
            p = inorder(root->left, parent, true);
        }
        p->right = root;
        p->left = nullptr;
        p = root;
        if (root->right)
        {
            p = inorder(root->right, root, false);
        }
        return p;
    }
};
...
int main()
{
    Solution solution;
    Codec c;
    TreeNode* root = c.deserialize("2,1,4,null,null,3,null");
